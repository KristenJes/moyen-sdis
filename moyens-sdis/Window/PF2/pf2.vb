...
retour:
        ...
        Me.Text = "Composition du " & lblCaserne.Text & ": Départ"
            ...
            Case "CCF"
                ...
                caQuery = "SELECT SP_NOM, SP_PRENOM FROM POMPIER, PARTICIPE WHERE POMPIER.SP_MATRICULE=PARTICIPE.SP_MATRICULE AND FONCTION_OCCUPEE=8 AND DEP_ID=(SELECT DEP_ID FROM DEPART WHERE INTERV_ID=3 AND ENGIN_ID=53)"
                caCCF = OracleUtils.Table(caQuery)

                Dim condCCF As DataTable
                condQuery = "SELECT SP_NOM, SP_PRENOM FROM POMPIER, PARTICIPE WHERE POMPIER.SP_MATRICULE=PARTICIPE.SP_MATRICULE AND FONCTION_OCCUPEE=14 AND DEP_ID=(SELECT DEP_ID FROM DEPART WHERE INTERV_ID=3 AND ENGIN_ID=53)"
                condCCF = OracleUtils.Table(condQuery)

                Dim ceCCF As DataTable
                ce1Query = "SELECT SP_NOM, SP_PRENOM FROM POMPIER, PARTICIPE WHERE POMPIER.SP_MATRICULE=PARTICIPE.SP_MATRICULE AND FONCTION_OCCUPEE=18 AND DEP_ID=(SELECT DEP_ID FROM DEPART WHERE INTERV_ID=3 AND ENGIN_ID=53)"
                ceCCF = OracleUtils.Table(ce1Query)

                Dim equCCF As DataTable
                equQuery = "SELECT SP_NOM, SP_PRENOM, PARTICIPE.SP_MATRICULE FROM POMPIER, PARTICIPE WHERE POMPIER.SP_MATRICULE=PARTICIPE.SP_MATRICULE AND FONCTION_OCCUPEE=3 AND DEP_ID=(SELECT DEP_ID FROM DEPART WHERE INTERV_ID=3 AND ENGIN_ID=53)"
